> **Active scope:** Working checklist of raw `DeprecationWarning` callsites that must be migrated to the centralized `deprecate()` helpers before v1.0.0. Moves to `finished-work` when all listed callsites are migrated and a scan confirms zero non-helper deprecation emits.
