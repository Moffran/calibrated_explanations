...
Legacy builder/renderer must wrap _plots_legacy so fallback preserves behaviour. Current LegacyPlotBuilder just returns the context and LegacyPlotRenderer returns an empty result without calling the legacy plotting helpers, so any fallback to the legacy plugin produces no figure.	ADR requirement vs. implementation and usage in .plot() fallback chain	5 – Critical	Wire the legacy plugin through _plots_legacy (or equivalent) before returning, ensuring parity when fallbacks occur.
...
Legacy builder/renderer must wrap _plots_legacy so fallback styles actually render.	LegacyPlotBuilder.build just returns the context and the renderer returns an empty result; no legacy drawing is triggered.	Critical	When non-legacy styles fall back to the legacy style (the default safety net) nothing is rendered, breaking .plot() parity that ADR-014 considers mandatory.
...
Recommended focus
Immediate fix: Rework the legacy plot builder/renderer so fallback execution truly mirrors _plots_legacy, restoring safe default behaviour.
