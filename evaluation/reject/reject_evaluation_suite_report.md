...
{0.3, 0.5, 0.7, 1.0}`, at epsilon `{0.05, 0.10}` across 26 binary datasets.
The question is whether any combination in the public-facing configuration space
breaks the coverage property that Scenario 1 established for the standard
operating point.

The result is clean: **0 total violations and 0 structural violations** across
the entire 416-row grid. Every `(NCF, w, epsilon, dataset)` combination passes.
This is the basis for treating the current binary NCF grid as coverage-safe for
deployment purposes, and it serves as a regression guard: any implementation
change that introduces violations in this sweep is a red flag.

There is a technical caveat worth noting: the coverage columns in the Scenario 7
result table show `nan` for many rows because coverage can only be computed when
prediction-set columns are label-index aligned. The violation detection logic
does handle this correctly — `nan` coverage rows are not flagged as violations —
but the reported aggregate of 0 violations reflects that aligned rows all pass,
not that every row was assessed. This subtlety is documented in the artifacts.
...
Regression reject in CE operates by threshold: instances whose predicted value
(or interval) crosses a user-supplied threshold are treated differently. The
scenario tests whether this threshold-based selection also happens to select
instances by *uncertainty* (interval width). The null hypothesis — that threshold
reject is purely value-based, not uncertainty-based — is confirmed resoundingly.

Across 22 regression datasets, threshold quantiles `{0.10, …, 0.50}`, and
confidence levels `{0.90, 0.95}`:

- Mean interval-width delta: **−0.0000** (effectively zero to four decimal
  places).
- Mean reject rate: **0.2083**.

The accepted-subset interval width equals the full-set interval width. This is
the expected null result because threshold reject partitions the dataset by
predicted response value, not by the width of the conformal interval. Instances
with predicted values above or below the threshold happen to have a wide range
of interval widths, so the two subsets end up with identical widths on average.

This scenario is a guardrail, not a failure. It documents that threshold
regression reject should be used only when the operational question is "should
we act on instances predicted to be below/above a threshold?" not "should we
defer uncertain regression predictions?" The latter requires a different
mechanism (difficulty-normalised regression reject, which is deferred to a
separate scenario pending the RT-2 fix).
...
Scenario 3 says: use threshold regression reject for threshold-event questions,
not for uncertainty selection. These are not weaknesses of the API — they are
...
| 7 | 26 binary | 0 total violations, 0 structural violations | Full NCF/weight grid passes coverage check; valid regression guard. |
...
**Scenario 3.** Threshold regression reject selects by predicted value, not by
uncertainty. Use it for threshold-event questions; label it heuristic.
...
**Scenario 7.** The full binary NCF/weight grid passes the coverage sweep.
Retain as a regression guard for implementation changes.
...
**Promote as stable.** The baseline binary conformal reject path (`builtin.default`,
NCF `default` or `ensured`, `w ≥ 0.7`) has a clean coverage record (Scenario 7)
and moderate finite-sample behavior (Scenario 1). It is the correct choice when
validity is the primary requirement.

**Treat as a scoped empirical tool.** The multiclass top-1 correctness proxy
(Scenario 2) and threshold regression reject (Scenario 3) are useful but must
be used within their stated semantics. Report singleton precision/recall for
multiclass; label threshold regression as heuristic.
