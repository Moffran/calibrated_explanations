...
test_size = 0.1  # number of test samples per dataset
is_debug = True
    ...
    for c1, c2, alg, X in model_struct:
        ...
        for cal_size in calibration_sizes:
            ...
            for sample_percentile in sample_percentiles:
                ...
                # print(f'{ct:.1f}',end='\t')
                ablation["ce"][cal_size][str(sample_percentile)].append(
                    [f.feature_weights for f in factual_explanations]
                )
                ...
                # print(f'{ct:.1f}',end='\t')
                ablation["cce"][cal_size][str(sample_percentile)].append(
                    [f.feature_weights for f in factual_explanations]
                )
