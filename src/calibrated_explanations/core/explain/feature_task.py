...
def _build_empty_feature_result(
    feature_index: int,
    n_instances: int,
    feature_values_list: list[Any],
    x_column: Any,
    predict: Any,
    low: Any,
    high: Any,
) -> FeatureTaskResult:
    """Return the standard empty-feature payload for early exits."""
...
def _process_categorical_feature(
    feature_index: int,
    n_instances: int,
    feature_values_list: list[Any],
    x_column: Any,
    ...
    feature_indices: Any,
    perturbed_feature: Any,
    value_counts_cache: Optional[Dict[Any, int]],
) -> FeatureTaskResult:
    """Compute feature-task outputs for a categorical feature."""
    weights_predict = np.zeros(n_instances, dtype=float)
    weights_low = np.zeros(n_instances, dtype=float)
    weights_high = np.zeros(n_instances, dtype=float)
    predict_matrix = np.zeros(n_instances, dtype=float)
    ...
    value_indices = np.array([value_to_index.get(row[2], -1) for row in feature_slice], dtype=int)
...
def feature_task(args: Tuple[Any, ...]) -> FeatureTaskResult:
    ...
    if feature_index in features_to_ignore_set:
        return _build_empty_feature_result(
            feature_index,
            n_instances,
            feature_values_list,
            x_column,
    ...
    if feature_index in categorical_features_set:
        return _process_categorical_feature(
            feature_index=feature_index,
            n_instances=n_instances,
            feature_values_list=feature_values_list,
            x_column=x_column,
            predict=predict,
            low=low,
            high=high,
            ...
            value_counts_cache=value_counts_cache,
        )
