...
class ParallelBenchmark:
    ...
    def __init__(self, explainer: Any, X: Any):
    ...
    def _run_single(self, strategy: str, workers: int) -> None:
        ...
        try:
            ...
            # Run explanation
            _ = self.explainer.explain(self.X)
            
            duration = time.perf_counter() - start
            n_instances = len(self.X) if hasattr(self.X, "__len__") else 1
    ...
    def report(self) -> str:
        ...
        lines.append(f"{'Strategy':<12} | {'Workers':<8} | {'Duration (s)':<12} | {'Throughput (i/s)':<16}")
        lines.append("-" * 60)
