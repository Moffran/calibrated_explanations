...
class NarrativePlotPlugin:
    ...
    def plot(
        ...
        **kwargs : dict
        ...
        elif template and Path(template).is_absolute():
        ...
        for i, explanation in enumerate(explanations.explanations):
            ...
            for level in levels:
                try:
                    ...
                    row[f"{explanation_type}_explanation_{level}"] = f"Error generating narrative: {e}"
    ...
    def _format_as_html(self, results: List[Dict[str, Any]]) -> str:
        ...
        for key in first_result.keys():
            html.append(f"<th style='padding: 8px; text-align: left;'>{key.replace('_', ' ').title()}</th>")
