...
def _create_valid_instruction_tree(root: Path) -> None:
    _write(
...
""".strip(),
    ...
    _write(
        root / "AGENTS.md",
        """
> canonical `AGENT_INSTRUCTIONS.md` is the single source of truth.
...
**Durable fix:** AGENT_INSTRUCTIONS.md
...
""".strip(),
    ...
    _write(
        ...
        """
See `AGENT_INSTRUCTIONS.md`, `AGENTS.md`, and `tests/README.md`.
""".strip(),
    )
    _write(
...
def test_checker_passes_on_valid_instruction_tree(tmp_path: Path) -> None:
    _create_valid_instruction_tree(tmp_path)
...
def test_checker_fails_on_coveragerc_and_bare_should(tmp_path: Path) -> None:
    _create_valid_instruction_tree(tmp_path)
    _write(
        tmp_path / "AGENT_INSTRUCTIONS.md",
