...
def _train_classification(n_classes=2, seed=0):
    X, y = make_classification(n_samples=200, n_features=6, n_informative=4, n_classes=n_classes, random_state=seed)
...
def _train_regression(seed=0):
    X, y = make_regression(n_samples=200, n_features=6, n_informative=4, noise=5.0, random_state=seed)
    X_proper, X_cal, y_proper, y_cal = train_test_split(X, y, test_size=0.25, random_state=seed)
    model = BayesianRidge()
    w = WrapCalibratedExplainer(model)
    w.fit(X_proper, y_proper)
