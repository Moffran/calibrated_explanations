    ...
    try:
        ...
        class StubDifficulty:
            ...
            def predict(self, x_data):
                ...
                # return ones (easy examples) for all points to avoid division by zero
                return _np.ones(len(x_data))
...
def test_safe_fit_difficulty_fallback(monkeypatch):
    ...
    assert np.allclose(stub.predict(np.ones((2, 2))), 1.0)
    assert np.allclose(stub.apply(np.ones((2, 2))), 1.0)
