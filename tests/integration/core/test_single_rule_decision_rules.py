...
def top_rule_feature(explanation) -> int:
...
def test_single_split_classification_recovers_rule():
    ...
    x, latent, feature_names = singlesplit_dataset()
    ...
    top_feature = top_rule_feature(factual[0])
    ...
    top_feature = top_rule_feature(alternative[0])
    assert top_feature < len(feature_names) // 2
...
def test_single_split_regression_recovers_rule():
    ...
    x, latent, feature_names = singlesplit_dataset(noise=0.02)
...
def test_single_split_probabilistic_regression_rule_and_intervals():
    ...
    x, latent, feature_names = singlesplit_dataset(noise=0.1)
