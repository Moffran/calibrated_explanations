...
from calibrated_explanations.utils.exceptions import NotFittedError, ConfigurationError
...
def test_collection_to_batch_with_items():
    ...
    batch = builtins._collection_to_batch(collection)
    assert batch.explanation_cls == type(exp1)
...
def test_legacy_interval_calibrator_plugin_create_regression():
    ...
    with patch("calibrated_explanations.calibration.interval_regressor.IntervalRegressor") as MockRegressor:
        plugin.create(context)
        MockRegressor.assert_called_once()
