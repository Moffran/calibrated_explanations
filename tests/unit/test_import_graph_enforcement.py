        ...
        for node in ast.iter_child_nodes(tree):
            ...
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                if isinstance(node, ast.ImportFrom):
                    module = node.module or ''
                    if node.level > 0:  # Relative import
                        if module in {'perf', 'plotting', 'plugins', 'explanations', 
                                     'integrations', 'discretizers'}:
                            module_level_imports.append(module)
                elif isinstance(node, ast.Import):
                    for alias in node.names:
                        if any(sibling in alias.name for sibling in 
                              ['perf', 'plotting', 'plugins', 'explanations']):
    ...
    def test_should_find_no_circular_imports_in_top_level_packages(self):
        ...
        for from_pkg, to_pkg in forbidden_cycles:
            ...
            for imp_module, lineno in imports:
                ...
                if top_level == to_pkg:
                    violations.append(
                        f"Forbidden cycle: {from_pkg} → {to_pkg} (line {lineno})"
                    )
        
        assert not violations, f"Circular imports detected:\n" + "\n".join(violations)
...
class TestImportGraphRuntime:
    ...
    @pytest.mark.parametrize('module_path,expected_accessible', [
