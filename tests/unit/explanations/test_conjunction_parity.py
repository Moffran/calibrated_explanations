...
def test_factual_conjunction_parity(binary_dataset, instance_index):
    explainer, x_test = _make_explainer(binary_dataset)

    explanation = explainer.explain_factual(x_test[instance_index].reshape(1, -1))
    explanation_legacy = explainer.explain_factual(x_test[instance_index].reshape(1, -1))
...
@pytest.mark.parametrize("instance_index", [0, 1])
def test_alternative_conjunction_parity(binary_dataset, instance_index):
    explainer, x_test = _make_explainer(binary_dataset)

    explanation = explainer.explore_alternatives(x_test[instance_index].reshape(1, -1))
    explanation_legacy = explainer.explore_alternatives(x_test[instance_index].reshape(1, -1))
