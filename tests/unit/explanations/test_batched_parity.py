            ...
            for j in range(len(v1)):
                if isinstance(v1[j], (np.ndarray, list)):
                    if isinstance(v1[j], list):
                        ...
                        for l in range(len(v1[j])):
                            if isinstance(v1[j][l], np.ndarray):
                                np.testing.assert_allclose(v1[j][l], v2[j][l])
                            elif isinstance(v1[j][l], float):
                                assert v1[j][l] == pytest.approx(v2[j][l])
                            else:
                                assert v1[j][l] == v2[j][l]
                    else:
