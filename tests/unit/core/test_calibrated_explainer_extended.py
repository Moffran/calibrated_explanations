...
def test_predict_proba_uncalibrated_multiclass_uq():
    ...
    with patch.object(CalibratedExplainer, 'is_multiclass', return_value=True):
        ...
        proba, (low, high) = expl.predict_proba(np.zeros((1, 1)), calibrated=False, uq_interval=True)
...
def test_predict_uncalibrated_regression():
    ...
    with patch("calibrated_explanations.api.params.warn_on_aliases"), \
         ...
         patch("calibrated_explanations.core.prediction_helpers.handle_uncalibrated_regression_prediction") as mock_handle:
...
def test_parallel_pool_lifecycle():
    ...
    with patch("calibrated_explanations.parallel.ParallelExecutor") as MockExecutor, \
