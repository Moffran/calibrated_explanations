...
def test_fallback_from_bulk_predict_set_to_per_instance(orchestrator):
    ...
    with pytest.warns(
        UserWarning, match="Reject prediction fallback engaged: bulk predict_set failed"
    ):
        breakdown = orchestrator.predict_reject_breakdown(x, confidence=0.9)
...
def test_fallback_from_bad_shape_to_per_instance(orchestrator):
    ...
    with pytest.warns(
        UserWarning,
        match="Reject prediction fallback engaged: predict_set returned unexpected shape",
    ):
        breakdown = orchestrator.predict_reject_breakdown(x, confidence=0.9)
