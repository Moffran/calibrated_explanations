...
def test_clean_condition_variants():
    ...
    original_sub = re.sub
    try:
        def mock_sub(*args, **kwargs): raise Exception("regex fail")
        re.sub = mock_sub
    ...
    gen.templates = {
        "narrative_templates": {
            "binary_classification": {
                "factual": {
                    "beginner": "Template {label} {calibrated_pred}"
                }
            }
        }
    }
...
def test_expand_template_caution_logic():
    ...
    res = gen._expand_template(template, pos_features, [], [], context, "beginner", "binary_classification")
    assert "⚠️ Use caution: uncertainty is high." in res
    
    # Advanced level caution
    res2 = gen._expand_template(template, pos_features, [], [], context, "advanced", "binary_classification")
