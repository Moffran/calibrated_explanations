...
def test_explanation_orchestrator_should_set_logging_context():
    ...
    with (
        patch.object(ExplanationOrchestrator, "resolve_plugin", side_effect=mock_resolve),
...
def test_prediction_orchestrator_should_set_logging_context():
    ...
    with (
        patch.object(PredictionOrchestrator, "resolve_interval_plugin", side_effect=mock_resolve),
        patch.object(PredictionOrchestrator, "ensure_interval_runtime_state"),
        patch.object(PredictionOrchestrator, "gather_interval_hints"),
        contextlib.suppress(Exception),
    ):
...
def test_prediction_orchestrator_should_set_plugin_identifier_context():
    ...
    with (
        patch.object(
            ...
            "resolve_interval_plugin",
            return_value=(plugin, "resolved-plugin"),
        ),
        patch.object(PredictionOrchestrator, "ensure_interval_runtime_state"),
        patch.object(PredictionOrchestrator, "gather_interval_hints"),
        patch.object(PredictionOrchestrator, "build_interval_context"),
        contextlib.suppress(Exception),
    ):
        orchestrator.obtain_interval_calibrator(fast=False, metadata={})
