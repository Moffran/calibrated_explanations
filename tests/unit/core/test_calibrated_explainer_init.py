...
@patch(
    "calibrated_explanations.core.calibration_helpers.identify_constant_features", return_value=[]
)
...
def test_deepcopy(mock_learner, basic_data):
    ...
    with patch(
        "calibrated_explanations.core.calibrated_explainer.CalibratedExplainer.__init__",
        return_value=None,
    ):
...
def test_plugin_preferences_enforcement(mock_pm_cls):
    with patch(
        ...
        assert (
            manager.explanation_plugin_overrides["factual"] == "core.explanation.factual.sequential"
        )
