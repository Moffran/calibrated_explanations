...
def _make_exp(prediction=None, rules=None):
    return Explanation(
        task="t",
        index=0,
...
def test_scalar_low_gt_high_raises():
    exp = _make_exp({"predict": 1, "low": 2, "high": 0})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)


def test_scalar_predict_outside_raises():
    exp = _make_exp({"predict": 10, "low": 0, "high": 5})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)


def test_vector_requires_vector_low_high():
    exp = _make_exp({"predict": [1, 2], "low": 0, "high": [0, 2]})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)


def test_vector_length_mismatch_raises():
    exp = _make_exp({"predict": [1, 2], "low": [0], "high": [2, 3]})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)


def test_vector_non_numeric_entries_raises():
    exp = _make_exp({"predict": [1, "x"], "low": [0, 0], "high": [2, 2]})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)


def test_vector_low_gt_high_at_index_raises():
    exp = _make_exp({"predict": [1, 2], "low": [0, 3], "high": [2, 4]})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)


def test_vector_predict_outside_at_index_raises():
    exp = _make_exp({"predict": [0, 5], "low": [0, 0], "high": [1, 4]})
    with pytest.raises(ValidationError):
        serialization.to_json(exp)
...
def test_rule_and_instance_prediction_checks():
    ...
    exp = _make_exp(prediction={}, rules=[fr])
    with pytest.raises(ValidationError):
        serialization.to_json(exp)
