...
MOCK_TEMPLATES = {
    "reject_indicators": {
        ...
        "outlier": {
            "beginner": "This looks like an outlier.",
            "advanced": "Outlier detected."
        }
    },
    "narrative_templates": {
        "classification": {
            "factual": {
                "advanced": "Base narrative.",
                "beginner": "Simple base narrative."
            }
        }
    }
}
...
class FakeExplanation:
    ...
    def get_rules(self):
        # Return minimal rules to satify generate_narrative
        default = {
            "base_predict": 0.5,
...
@pytest.fixture
def generator():
    with patch("calibrated_explanations.core.narrative_generator.load_template_file", return_value=MOCK_TEMPLATES):
...
def test_narrative_handles_formatting_errors_gracefully(generator):
    ...
    broken_templates = {
        "reject_indicators": {
            "ambiguity": {
                "advanced": "Sort of ambiguous: {missing_key}"
            }
        },
        "narrative_templates": MOCK_TEMPLATES["narrative_templates"]
    }
    
    with patch("calibrated_explanations.core.narrative_generator.load_template_file", return_value=broken_templates):
        gen = NarrativeGenerator("dummy_path.yaml")
