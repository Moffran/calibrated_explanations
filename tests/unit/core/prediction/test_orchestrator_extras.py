...
def test_predict_impl_fast_probabilistic_regression(orchestrator, mock_explainer):
    ...
    mock_learner.predict_probability.return_value = (
        np.array([0.5]), np.array([0.4]), np.array([0.6]), None
    )
...
def test_resolve_interval_plugin_missing_in_chain(orchestrator, mock_explainer):
    ...
    with patch("calibrated_explanations.core.prediction.orchestrator.find_interval_descriptor", return_value=None), \
         patch("calibrated_explanations.core.prediction.orchestrator.find_interval_plugin", return_value=None), \
         patch("calibrated_explanations.core.prediction.orchestrator.find_interval_plugin_trusted", side_effect=[None, valid_plugin_mock]), \
         patch("calibrated_explanations.core.prediction.orchestrator.is_identifier_denied", return_value=False):
...
def test_obtain_interval_calibrator_fast_metadata(orchestrator, mock_explainer):
    ...
    with patch.object(orchestrator, "_resolve_interval_plugin", return_value=(mock_plugin, "test_plugin")):
