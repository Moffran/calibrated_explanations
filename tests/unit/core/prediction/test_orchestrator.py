...
class TestPredictionOrchestratorBehavior:
    ...
    def test_predict_classification_returns_valid_probabilities(self, binary_classification_explainer):
    ...
    def test_predict_regression_with_intervals(self, regression_explainer):
        ...
        for p, l, h in zip(predictions, low, high):
            assert l <= p <= h or (np.isnan(l) or np.isnan(h))
