...
@pytest.mark.parametrize("metadata, expected_error", [
...
def test_check_explanation_runtime_metadata_errors(explanation_orchestrator, metadata, expected_error):
...
def test_check_explanation_runtime_metadata_success(explanation_orchestrator):
    ...
    assert explanation_orchestrator._check_metadata(metadata, identifier="plugin", mode="factual") is None

@pytest.mark.parametrize("metadata, fast, expected_error", [
    ...
    }, False, "missing capability"),
    ({
        "schema_version": 1,
        "modes": ["classification"],
        "capabilities": ["interval:classification"],
        "fast_compatible": False,
    }, True, "not marked fast_compatible"),
    ({
        "schema_version": 1,
        "modes": ["classification"],
        "capabilities": ["interval:classification"],
        ...
        "requires_bins": True,
    }, True, "requires bins"), # Assuming bins is None
])
def test_check_interval_runtime_metadata_errors(prediction_orchestrator, metadata, fast, expected_error):
    error = prediction_orchestrator._check_interval_runtime_metadata(metadata, identifier="interval", fast=fast)
    assert error is not None
    assert expected_error in error
...
def test_check_interval_runtime_metadata_success(prediction_orchestrator):
    ...
    assert prediction_orchestrator._check_interval_runtime_metadata(metadata, identifier="interval", fast=True) is None
