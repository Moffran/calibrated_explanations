...
Covers all 9 in-scope Task-10 findings:
  ...
  2.   perturbation.py singleton lifecycle fixed
...
# ---------------------------------------------------------------------------
# Finding 2 — perturbation.py singleton lifecycle
# ---------------------------------------------------------------------------


def test_should_expose_reset_perturbation_config_manager_for_testing() -> None:
    assert callable(reset_perturbation_config_manager_for_testing)


def test_reset_perturbation_config_manager_is_callable_and_does_not_raise() -> None:
    # Behavioral: reset must succeed without raising regardless of prior state.
    assert reset_perturbation_config_manager_for_testing() is None
    assert reset_perturbation_config_manager_for_testing() is None  # idempotent
