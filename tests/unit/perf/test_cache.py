...
def test_cache_fallback_when_cachetools_unavailable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def mock_import(name, *args, **kwargs):
        if name == "cachetools":
            raise ImportError("No module named 'cachetools'")
    ...
    assert hasattr(cache_module, "LRUCache")
    assert hasattr(cache_module, "TTLCache")

    # Test basic LRUCache functionality
    cache = cache_module.LRUCache(maxsize=2)
    cache["key1"] = "value1"
    cache["key2"] = "value2"
    ...
    ttl_cache = cache_module.TTLCache(maxsize=2, ttl=1.0)
