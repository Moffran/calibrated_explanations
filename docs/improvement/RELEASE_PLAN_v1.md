...
11. **Sustain ADR-019 coverage uplift.** Land the coverage uplift actions queued for v0.10.0: finish the waiver audit, lock the CI gate at `--cov-fail-under=90` with per-module expectations for plotting/viz builders and the plugin stack, and ship the Phase-2 remediation work (Iteration 3 in the uplift plan) that hardens `plugins/registry.py`, `plugins/builtins.py`, `plugins/cli.py`, and legacy plotting delegation via the new regression tests and Windows/Posix parity checks outlined in the plan.【F:docs/improvement/coverage_uplift_plan.md†L63-L119】
...
7. Promote ADR-024/ADR-025/ADR-026 from Draft to Accepted with implementation
   ...
   freeze.【F:docs/improvement/adrs/ADR-024-plotspec-inputs.md†L1-L80】【F:docs/improvement/adrs/ADR-025-plotspec-rendering.md†L1-L90】【F:docs/improvement/adrs/ADR-026-explanation-plugins.md†L1-L86】
