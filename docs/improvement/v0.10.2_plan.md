...
## 9) ADR-029 reject integration strategy (documentation decisions)

**Goal:** Publish the reject integration ADR decisions, capture deferred strategy/visualization questions, and document follow-up alternatives without changing runtime behavior.
...
2. Record open questions and alternatives for strategy extensibility (B2) and visualization so future iterations can make an informed choice.
3. Link ADR-029 from the v0.10.2 release milestone in `RELEASE_PLAN_v1.md`.
...
**Decisions (sequenced)**
1. Strategy extensibility: adopt B2 (registry) only.
2. Visualization integration: D1 (no visualization integration).

**Open questions and alternatives (for follow-up)**
1. **Invocation model**
   - Define `RejectPolicy` enum members (e.g., `NONE`, `SUBMIT`).
