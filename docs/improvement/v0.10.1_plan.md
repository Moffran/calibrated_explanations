...
**Implementation steps**
1. **Inventory Pattern 2 & 3 candidates.** [DONE]
...
2. **Remediate Pattern 3 (Dead Code).** [DONE]
...
3. **Remediate Pattern 2 (Test Utilities).** [DONE]
   ...
   - Note: Name-mangled members (e.g., `_CalibratedExplainer__initialized`) are
     deferred to Pattern 1 remediation as they represent library internals.

4. **Inventory Category A calls.** [DONE]
   - Enumerate remaining private-member usage under `tests/` (Pattern 1).
   - Classify Category A calls by severity/complexity for phased refactoring.
   - Prioritize targets that block other work or have high maintenance cost.
   - Created detailed [Pattern 1 Remediation Plan](PATTERN_1_REMEDIATION_PLAN.md).

5. **Refactor tests to public APIs (Category A).** [IN PROGRESS]
   - Update tests to use `CalibratedExplainer` / `WrapCalibratedExplainer`
     public methods rather than direct private access.

6. **Enforce and validate.** [IN PROGRESS]
   - Updated `.github/private_member_allowlist.json` with 93 baseline entries.
   - Add a CI guard to prevent new private-member usage in tests.

**Decisions**
- Use a targeted allowlist with explicit expiry versions for the CI guard during
...
**Verification checklist**
- Pattern 3 (Dead Code) count is 0. [DONE]
- Pattern 2 (Test Utilities) count is 0 (excluding name-mangled internals). [DONE]
- Category A tests no longer call private members. [IN PROGRESS]
