...
1. **Resolve naming drift (choose one approach).**
      - Decision (v0.11.0):
         ...
         - Commit the ADR-004 addendum document to `docs/improvement/adrs/` and
            link it from the public API docs and `CHANGELOG.md`. Publish a
            deprecation-window policy in the addendum describing how long aliases
            will remain supported before breaking changes are considered.
