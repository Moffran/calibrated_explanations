...
`ConfigurationError` at the `explain_factual(guarded=True)` /
`explore_alternatives(guarded=True)` call site.  See ADR-032, decision 7 for the rationale.
```

For guarded regression workflows, start from {doc}`quickstart_regression`.
The only workflow difference is passing `guarded=True` to the explanation call:

- Use `explain_factual(..., guarded=True)` instead of `explain_factual(...)`.
- Use `explore_alternatives(..., guarded=True)` instead of `explore_alternatives(...)`.
...
```python
guarded_factual = explainer.explain_factual(x_test[:5], guarded=True, significance=0.1)
guarded_alts = explainer.explore_alternatives(x_test[:2], guarded=True, significance=0.1)
```
