...
config = (
    ...
    .perf_parallel(True, backend="threads", workers=4, min_batch=16)
    .build_config()
)
wrapper = WrapCalibratedExplainer.from_config(config)
```
...
- `perf_cache` and `perf_parallel` require `enabled: bool` as the first
  positional argument.
- `perf_parallel(True)` also requires an explicit `backend=` (`"threads"`,
  `"processes"`, `"joblib"` or `"sequential"`); without one `build_config()`
  raises `ConfigurationError` (ADR-004).
...
```bash
# Minimal — enable with an explicit strategy
CE_PARALLEL=on,threads

# With inline tokens (single env var value)
CE_PARALLEL=on,workers=4,min_batch=16,threads
```

A strategy token (`threads`, `processes`, `joblib` or `sequential`) is
required: `CE_PARALLEL=on` alone leaves the removed `auto` strategy selected
and raises `ConfigurationError` at configuration time.
...
inline: `CE_PARALLEL=on,threads,workers=4`.
