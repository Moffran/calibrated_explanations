...
| `CE_CACHE` | Performance | `off` (default) or `on[,max_items=N,max_bytes=N,ttl=N]` — see [caching topic](#6-caching) |
| `CE_PARALLEL` | Performance | `off` (default) or `on[,workers=N,min_batch=N,threads]` — see [parallel topic](#7-parallel) |
...
### Deprecated in v0.11.3 — removal target v1.0.0

| Deprecated symbol | Canonical replacement |
|---|---|
| `CalibratedExplainer.explain_guarded_factual(...)` | `explainer.explain_factual(..., guarded=True)` |
| `CalibratedExplainer.explore_guarded_alternatives(...)` | `explainer.explore_alternatives(..., guarded=True)` |
| `WrapCalibratedExplainer.explain_guarded_factual(...)` | `wrapper.explain_factual(..., guarded=True)` |
| `WrapCalibratedExplainer.explore_guarded_alternatives(...)` | `wrapper.explore_alternatives(..., guarded=True)` |

Set `CE_DEPRECATIONS=error` in CI to surface any remaining uses as hard
errors:
...
The `explain_guarded_factual` / `explore_guarded_alternatives` methods are
deprecated as of v0.11.3 and will be removed in v1.0.0.
...
The `guarded=True` flag enables calibration-feature divergence checks (ADR-032).
Behaviour is identical to the removed methods; only the call signature changed.
